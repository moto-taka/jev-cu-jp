...
if (!cases.length) {
  console.error("評価ケースがありません。fixtures/p0/cases.json を確認してください");
  process.exit(1);
}
...
const cost = rows.every((r) => typeof r.cost === "number")
  ? rows.reduce((a, r) => a + r.cost, 0)
  : null;
const p50 = lat.length ? lat[Math.floor(lat.length / 2)] : null;

console.log(
  `\n正解率 ${ok.length}/${rows.length}` +
    ` · p50 ${p50 ?? "n/a"}ms` +
    ` · tokens ${tokens}` +
    ` · 費用 ${cost === null ? "取得できず" : `≈ $${cost.toFixed(6)}`}`,
);
